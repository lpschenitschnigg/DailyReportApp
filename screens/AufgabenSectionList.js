...
class AufgabenSectionList extends Component {
    ...
    static navigationOptions =({navigation}) => {
        ...
        return {
            ...
            title: 'Aufgaben',
            headerTitle: <Text style={{ fontFamily: 'siemens_global_bold',fontSize: 28, lineHeight: 34, color: "#009999", left: 20}}>Statusmeldungen</Text>,
    ...
    componentWillMount() {
        ...
        console.log(j, m, t);
        var vorgestern = moment(d).subtract(2, 'day').format('YYYY-MM-DD');
        console.log(vorgestern);
        var vj = parseInt(vorgestern.substring(0, 4), 10);
        var vm = parseInt(vorgestern.substring(5, 7), 10);
        var vt = parseInt(vorgestern.substring(8, 10), 10);
        this.setState({vorgestern: vorgestern});
        console.log("vorgestern", vorgestern);
        console.log(vj, vm, vt);
        ...
        //     console.log(aufgaben);
        //     //this._color(aufgaben);
        //     this.state.dataW.reverse(); // https://stackoverflow.com/questions/37435334/correct-way-to-push-into-state-array
        //     var joined = this.state.dataW.concat(aufgaben);
        //     this.setState({dataW: joined});
        //     this.state.dataW.reverse();
        //     //console.log(aufgaben);
        // })
            ...
            return (
                <View>
                    <View style={{paddingLeft: 18, borderBottomWidth: 1, borderBottomColor: '#EEEEEE', backgroundColor: '#fff', paddingBottom:15}}>
                        <Text onPress={() => this._detail(item)} style={{fontSize: 16, lineHeight: 24, paddingTop: 12, fontFamily: 'siemens_global_bold'}}>
                            {item.title}
                        </Text>
                        <Text onPress={() => this._detail(item)} style={{ fontSize: 12, lineHeight: 12, color: '#505050', fontFamily: 'siemens_global_roman'}}>
                            {item.content}
                        </Text>
                        <Text onPress={() => this._detail(item)} style={{ fontSize: 12, lineHeight: 16, color: '#505050', fontFamily: 'siemens_global_roman'}}>
                            {item.type}
                        </Text>
                    </View>
                    <View style={{position: 'absolute', right: 20, justifyContent: 'center', alignItems: 'flex-end', top: 25}}>
                        <Text onPress={() => this._detail(item)} style={{ fontSize: 12, lineHeight: 16, color: '#505050', fontFamily: 'siemens_global_bold'}}>
                            {item.from.substring(8, 10)}.{item.from.substring(5,7)}.{item.from.substring(0, 4)}
                        </Text>
                        <Text onPress={() => this._detail(item)} style={{ fontSize: 14, lineHeight: 14, color: '#505050', fontFamily: 'siemens_global_roman'}}>{this._timefunction(item.to, item.from)}</Text>
                    </View>
                {/* <View style={{borderBottomWidth: 1, borderBottomColor: '#EAEAEA'}}></View> */}
                </View>
            )
